...
nosave nomask mixed _query( mapping map, string *parts )
{
    mixed value;
    int i, s;
...
nosave nomask int _delete( mapping map, string *parts )
{
    if( sizeof( parts ) == 1 )
    {
...
nosave nomask mixed _set( mapping map, string *parts, mixed value )
{
    mixed old_value;
